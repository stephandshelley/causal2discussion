...
just do general L and A and Y, all t should be k
$\bar_L(k)$ = f_{L(k)}(\bar{U_{L(t)}}, \bar{A}(k-1), \bar{L}(k))
$\bar_A(k)$ = f_{A(k)}(\bar{U_{A(t)}}, \bar{A}(k-1), \bar{L}(k))
Y(3) = f_{Y(k+1)}(\bar{U_{Y(t+1)}}, \bar{A}(k), \bar{L}(k))
...
In order to identify the average treatment effect from the observed data distribution in a single point treatment context, we would need to verify the randomization assumptions (that $\bar{a(t)}|L$ is independent of Y, the independence assumption (met by conditioning on L so that all U's (U_A independent of U_Y and U_L is independent of U_Y) are independent of one another), and the positivity assumption min_aEA P(A=a|L)>0. 
