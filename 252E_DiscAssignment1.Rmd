...
& + \alpha_4l_k + \alpha_4l_{k-1} + \alpha_5l_{k-1} + \alpha_6a_{k-1}l_k \\
& + \alpha_7l_0 \\
\end{align*}
...
How does detectability of HIV RNA in patients' blood vary as a function of AZT dose history?
...
O={L(0), L(1), A(0), A(1), Y(2)}
L-bar = all measured risk factors for Y (CD4 count, age, white blood count, hematocrit, AIDS diagnosis, symptoms
A-bar = AZT dosage (100mg)
Y = HIV RNA detected in blood
...
A(1) = fA(UA(1), L(1))
L(2) = fL(UL(2), A(1), L(1))
A(2) = fA(UA(2), A(1), L(1), L(2))
=f(U, A(2), A(1), L(2), L(1))
Y(3) = fY(UY, A(2), A(1), L(2), L(1), )


# Question 3: Specify the counterfactuals of interest. How are these counterfactuals generated using an NPSEM?
Our counterfactuals of interest are the values of $Y_{a(t)}$ we would have observed if $A_0$ had been set to values 0, 1, 2, 3,...15 at each time point, all the 16 possible AZT doses at each time point. We are able to generate these counterfactuals using a parsimonious dose-response relationship specified in a linear logistic marginal structural model: 
\begin{align}
logit pr[Y_{\bar{a_0}} =1] = \beta_0 + \beta_1\sum_{t=1}^Ka(t)
\end{align}


# Question 4: Specify the target causal parameters. Discuss their interpretation.
\begin{align}
E(Y_{\bar{a}})=m(\bar{a}|\beta= \beta_0 + \beta_1\sum_{t=1}^ka(t)
\end{align}

Our target causal parameter is $E(Y_{\bar{a}})$ which is defined by the above equation. In this model, $e^\beta_1$ is the parameter that would give us the causal OR for HIV RNA detection associated with an increasing AZT dose of 100mg. $\beta_1$ represents the causal parameter if the relationship between AZT dose and HIV RNA is unconfounded by measured or unmeasured factors.  

# Question 5: What are the observed data? What is the link between the observed data and the structural causal model $\mathcal{M}^F$? Do we place any restrictions on theobserved data model $\mathcal{M}$?

\begin{align}
\Psi^F(P_{U,X}) = \Psi(P_0)
O=(\bar{A(t)}, \bar{L(t)})~P_0
...
In order to identify the average treatment effect from the observed data distribution, we would need to verify the randomization assumptions (that $\bar{a(t)}$ is independent of Y), that all backdoor paths are closed between A and Y, and the positivity assumption. 
