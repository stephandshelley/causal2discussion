...
<!-- From Maya's slides, lect 6, slide 12, I think this is wrong, so I'm commenting it out (SH)
...
&P( L_2 | A_1 = a_1, L_1 = l_1, A_0 = a_0, L_0 = l_0) X \\
&P(A_1 | L_1 = l_1, A_0 = a_0, L_0 = l_0) X \\
&P(L_1 | A_0 = a_0, L_0 = l_0) X \\
&P(A_0 | L_0 = l_0) X \\
&P(L_0 = l_0) \\
\end{align*}
This is the same as, 
\begin{align*}
P(L_0, A_0, L_1, A_1, L_2) &= \\
\prod_{0}^{2} P(L_k | \bar{A}_{k-1} = \bar{a}_{k-1}, \bar{L}_{k-1} = \bar{l}_{k-1} ) X \\
...
1. Sequential Randomization assumption, which assumes each a is independent of y (and which allows us to control for a different set of covariates at each time point)
2. Positivity assumption, which assumes that no strata have few or no observations. 
...
2. Use a pooled logistic regression to model CHD using time-varying and baseline risk factors for the 2 previous time points (2 years and 4 years prior) among those with no prior diagnosis of CHD
